...
Провайдер по умолчанию — Gemini (`LMS_AI_PROVIDER=gemini`, `LMS_AI_MODEL=gemini-2.0-flash`), бесплатного тарифа достаточно для подготовки материалов; ключ и лимиты описаны в `.env.production.example`. Точки входа: кнопка «ИИ-помощник» в шапке карты курса, ссылка в меню и подсказки в формах блока, темы и задания (там же можно сразу выбрать существующую тему как получателя).
