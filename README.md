...
- **Гибридный режим.** С ключом провайдера (`LMS_AI_API_KEY`) материал разбирает ИИ: понимает фото страницы учебника, PDF, Word и Excel, а Gemini — ещё и видео с аудио. Без ключа помощник не выключается, а работает **офлайн**: сам извлекает текст из TXT/MD/CSV, DOCX, XLSX и PDF и раскладывает его по структуре эвристиками. Выключить целиком — `LMS_AI_ENABLED=0`.
- **Провайдер — одна переменная.** По умолчанию **GigaChat (Сбер)**: работает из России без VPN, для физлиц есть бесплатный режим Freemium, читает фото и PDF. В поставке также готовы адаптеры Yandex AI Studio, ProxyAPI, VseGPT, AITUNNEL, OpenRouter, DeepSeek, Qwen, локальный Ollama и Gemini (`LMS_AI_PROVIDER`); любой другой OpenAI-совместимый шлюз подключается парой `LMS_AI_PROVIDER` + `LMS_AI_ENDPOINT` без правок кода. Ключа нет — помощник разбирает материал офлайн. Проверить настройки на площадке: `python manage.py ai_check` (без сети) и `python manage.py ai_check --live` (один пробный запрос к провайдеру).
- **Черновики вместо публикации.** Импорт всегда создаёт задания со статусом «черновик»; публикует человек. Повторный импорт того же материала не плодит дубликаты.
- **Лимиты и приватность.** Ответ модели валидируется и обрезается по настройкам `LMS_AI_MAX_*` (блоки, темы, задания, вопросы, карточки, длина текста, размер файла). В журнал попадают только счётчики, содержимое работ и файлов не логируется.
- **Без новых зависимостей.** Запрос к провайдеру идёт через стандартный `urllib`, архивы Office читаются `zipfile`; `requirements.txt` с хешами не меняется, поэтому сборка Amvera остаётся прежней.

**Почему не Gemini по умолчанию.** Gemini API из России недоступен: ни ключ получить, ни запросы отправлять без VPN и зарубежного аккаунта. Поэтому провайдер по умолчанию — GigaChat (Сбер), который работает без VPN и бесплатен для физлиц; Gemini остался в списке на случай, если доступ появится. Полное сравнение бесплатных вариантов, лимиты, сертификат НУЦ Минцифры и рецепты настройки каждого провайдера — в [docs/AI_PROVIDERS.md](docs/AI_PROVIDERS.md), переменные — в `.env.production.example`. Точки входа: кнопка «ИИ-помощник» в шапке карты курса, ссылка в меню и подсказки в формах блока, темы и задания (там же можно сразу выбрать существующую тему как получателя).
...
На карте курса есть отдельная папка **«Архив»** и два варианта удаления. **Архивирование** блока, темы или задания скрывает его от учеников, но сохраняет сдачи и оценки; восстановление доступно с подтверждением. **Удаление навсегда** показывается для пустых элементов без истории сдач и также требует подтверждения. Объекты с дочерними элементами или попытками защищены от потери данных — для них используется архив.
