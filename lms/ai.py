...
- если ученик должен прочитать текст, посмотреть картинку, прослушать аудио или изучить
  файл, материал обязан быть доступен ученику в самом задании;
...
def build_prompt(text, *, prompt="", target="mixed", filename="", kind="text"):
    ...
    if text.strip():
        ...
        parts.append("В запросе нет отдельного материала, на который можно ссылаться в условии задания.")
